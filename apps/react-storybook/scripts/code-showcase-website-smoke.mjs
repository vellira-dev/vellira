...
try {
  for (const colorScheme of ['light', 'dark']) {
    for (const viewport of [
      ...
      const scenario = `${colorScheme}-${viewport.width}`;
      ...
      try {
        ...
        const control = (name) => picker.getByRole('button', { name, exact: true });
        const code = section.locator('pre code');

        await picker.scrollIntoViewIfNeeded();
        await expect(picker).toBeVisible();
        await expect(picker.getByRole('button')).toHaveCount(4);
        await expect(page.locator('[data-vellira-theme]').first()).toHaveAttribute(
          'data-vellira-theme',
          colorScheme
        );
        ...
        async function checkSelection(name) {
          ...
          for (const other of examples.filter((item) => item !== name)) {
            await expect(control(other)).toHaveAttribute('aria-pressed', 'false');
          }
          await expect(section.getByText(`${name}.tsx`, { exact: true })).toBeVisible();
          await expect(code).toHaveCount(1);
          await expect(code).toContainText(`<${name}`);
          await expect(section.getByRole('link', { name: 'Docs', exact: true })).toHaveAttribute(
            'href',
            `https://docs.vellira.dev/components/${name.toLowerCase()}`
          );
          const category = ['Button', 'Input'].includes(name) ? 'primitives' : 'components';
          await expect(section.getByRole('link', { name: 'Storybook', exact: true })).toHaveAttribute(
            'href',
        ...
        for (const name of ['Input', 'Modal', 'Dropdown', 'Button']) {
          ...
          await expect(control(name)).not.toHaveCSS('background-color', 'rgba(0, 0, 0, 0)');

          if (name === 'Input') {
            const input = section.getByRole('textbox', { name: 'Workspace name', exact: true });
            await input.fill('Preserved workspace');
            await expect(input).toHaveValue('Preserved workspace');
          } else if (name === 'Modal') {
            await section.getByRole('button', { name: 'Open modal', exact: true }).click();
            const dialog = page.getByRole('dialog', { name: 'Create workspace' });
            await expect(dialog).toBeVisible();
            await dialog.getByRole('button', { name: 'Cancel', exact: true }).click();
            await expect(dialog).not.toBeVisible();
          } else if (name === 'Dropdown') {
            await section.getByRole('button', { name: 'Actions', exact: true }).click();
            const item = page.getByRole('menuitem', { name: 'Edit workspace', exact: true });
            await expect(item).toBeVisible();
            await page.keyboard.press('Escape');
            await expect(item).not.toBeVisible();
          } else {
            await expect(section.getByRole('button', { name: 'Continue', exact: true })).toBeVisible();
          }

          await picker.screenshot({ path: `${artifactDir}/${scenario}-${name.toLowerCase()}.png` });
        }
        ...
        await expect(section.getByRole('textbox', { name: 'Workspace name', exact: true })).toHaveValue('Preserved workspace');
        ...
        assert.ok(focus.visible && focus.outline !== 'none' && parseFloat(focus.width) > 0, 'keyboard focus must remain visible');
        ...
        await section.getByRole('tab', { name: 'React Native', exact: true }).click();
        await expect(code).toContainText("from '@vellira-ui/react-native';");
        await expect(section.getByText('pnpm add @vellira-ui/react-native', { exact: true })).toBeVisible();
        await control('Dropdown').click();
        await checkSelection('Dropdown');
        await expect(code).toContainText("from '@vellira-ui/react-native';");
        await section.getByRole('tab', { name: 'React', exact: true }).click();
        await expect(code).toContainText("from '@vellira-ui/react';");
        await checkSelection('Dropdown');

        assert.deepEqual(pageErrors, [], 'unexpected page errors');
        evidence.push({ scenario, examples, keyboard: 'passed', platformSwitch: 'passed', pageErrors });
        console.log(`CodeShowcase browser regression: ${scenario} OK`);
      } catch (error) {
        await page.screenshot({ path: `${artifactDir}/${scenario}-failure.png`, fullPage: true }).catch(() => {});
        throw error;
      } finally {
        await context.close();
      }
