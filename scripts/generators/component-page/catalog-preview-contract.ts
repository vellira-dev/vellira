export type CatalogPreviewFindingCode =
  ...
  | 'catalog-preview.generated-demo-import'
  | 'catalog-preview.generated-playground-import';
...
export function getCatalogPreviewFindings(params: {
  ...
  generatedPreviewSources?: Readonly<Record<string, string>>;
  ...
  for (const slug of generatedPreviewSlugs) {
    const source = params.generatedPreviewSources?.[slug];
    ...
    if (/from\s+['"][^'"]*Demo['"]/.test(source)) {
      findings.push({
        code: 'catalog-preview.generated-demo-import',
        slug,
        message: `Generated signature preview "${slug}" must not import a Demo.`,
      });
    }

    if (/from\s+['"][^'"]*Playground['"]/.test(source)) {
      findings.push({
        code: 'catalog-preview.generated-playground-import',
        slug,
        message: `Generated signature preview "${slug}" must not import a Playground.`,
      });
    }
  }
