...
export function auditGeneratedCssBytes(params: {
  generated: string;
  committed: string;
}): FindingInput[] {
  if (params.generated === params.committed) return [];

  return [
    finding(
      'generated-css-stale',
      'packages/tokens/src/generated/tokens.css',
      `Committed tokens.css differs from generateTokenCss() (${params.committed.length} committed bytes vs ${params.generated.length} generated bytes).`,
      'Committed generated CSS must be byte-identical to the canonical #881 serializer output.'
    ),
  ];
}
...
function variablesFromOutput(
  output: Map<string, { variable: string; value: string }>
): string[] {
...
export function checkTokenValueKindOutputs(root: string): RuleResult {
  ...
  const committedCss = fs.existsSync(generatedCssPath)
    ? fs.readFileSync(generatedCssPath, 'utf8')
    : '';
  findings.push(...auditGeneratedCssBytes({ generated: generatedCss, committed: committedCss }));
