...
describe('state vocabulary completion', () => {
  it(
    'proves compound-state and platform renderer contracts without findings',
    () => {
  ...
  it(
    'covers every canonical state in selected composition and both platform mappings',
    () => {
      const expected = [...canonicalInteractionStates].sort();
      expect(Object.keys(selectedCompoundStateGrammarV1).sort()).toEqual(expected);
      expect(
        Object.keys(interactionStatePlatformContractV1.web).sort()
      ).toEqual(expected);
      expect(
  ...
  it(
    'does not require native hover and keeps physical press distinct from active',
    () => {
